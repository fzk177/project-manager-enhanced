...
/**
 * 交付日不计入工期时，实际可用工作截止到前一个工作日。
 */
function pmhPreviousWorkday(value) {
  const date = pmhDateValue(value);
  if (!date) return ``;

  do {
    date.setDate(date.getDate() - 1);
  } while (!pmhIsWorkday(date));
...
function pmhEffectiveTargetDate(value, included = true) {
  const targetDate = pmhValidDate(value);
  if (!targetDate) return ``;
  return included !== false ? targetDate : pmhPreviousWorkday(targetDate);
}

function pmhWorkdaysToTarget(startValue, targetValue, included = true) {
  const effectiveTargetDate = pmhEffectiveTargetDate(targetValue, included);
  return effectiveTargetDate ? pmhWorkdaysBetween(startValue, effectiveTargetDate) : 0;
}

function pmhTargetDatePassed(targetValue, included = true, today = pmhToday()) {
  const effectiveTargetDate = pmhEffectiveTargetDate(targetValue, included);
  return Boolean(effectiveTargetDate && effectiveTargetDate < today);
}

function pmDeliveryBoundaryIncluded(batch, phase) {
  return phase === `testing`
    ? batch?.includeTestingDate !== false
    : batch?.includeReleaseDate !== false;
}

function pmDeliveryBoundaryLabel(included) {
  return included === false ? `不含当日` : `含当日`;
}
...
/**
 * 总工时优先归属实际完成人；没有完成人时才回退到任务指派人。
 */
function pmhTaskWorkOwners(task) {
  const completedBy = String(task.completedBy ?? task.customFields?.completedBy ?? ``).trim();
  return completedBy ? [completedBy] : pmhTaskAssignees(task);
}
...
function pmDeliveryNormalizePlan(value) {
  ...
  return {
    ...
    batches: Array.isArray(source.batches) ? source.batches.map((batch) => ({
      ...
      // 旧数据按原有产能算法默认计入节点当日。
      includeTestingDate: batch?.includeTestingDate !== false,
      includeReleaseDate: batch?.includeReleaseDate !== false,
...
function pmhTaskPlannedProgress(task, projectDates, deliveryContext = null) {
  ...
  if (deliveryContext?.enabled) {
    ...
    if (kind === `testing`) {
      ...
      if (testingDate) start = testingDate;
      deadline = pmhValidDate(batch.releaseDate);
      deadlineIncluded = pmDeliveryBoundaryIncluded(batch, `release`);
    } else {
      deadline = pmhValidDate(batch.testingDate);
      deadlineIncluded = pmDeliveryBoundaryIncluded(batch, `testing`);
    }
  } else {
  ...
  const effectiveDeadline = pmhEffectiveTargetDate(deadline, deadlineIncluded);
  if (!effectiveDeadline) return null;

  const today = pmhToday();
  if (today < start) return 0;
  if (today >= effectiveDeadline) return 1;
  const total = pmhWorkdaysBetween(start, effectiveDeadline);
  if (total <= 0) return 1;
...
function pmhMemberHealth(name, tasks, projectDates, deliveryContext = null, nodeWorkload = null) {
  ...
  const fallbackRemaining = unfinished.reduce((total, task) => total + pmhTaskRemaining(task), 0);
  const requiresTesting = unfinished.some((task) => [`requirement`, `development`].includes(pmhTaskKind(task)));
  const fallbackTarget = requiresTesting && projectDates.testing ? projectDates.testing : projectDates.completion;
  const fallbackPhase = requiresTesting ? `testing` : `release`;
  const fallbackDateField = fallbackPhase === `testing` ? `testingDate` : `releaseDate`;
  const fallbackBatches = deliveryContext?.batches?.filter((batch) => (
    pmhValidDate(batch[fallbackDateField]) === fallbackTarget
    && (deliveryContext.scopeId === PM_DELIVERY_SCOPE_ALL || batch.id === deliveryContext.scopeId)
  )) ?? [];
  const fallbackTargetIncluded = fallbackBatches.length > 0
    ? fallbackBatches.every((batch) => pmDeliveryBoundaryIncluded(batch, fallbackPhase))
    : true;
  const workloadNode = Array.isArray(nodeWorkload?.nodeRisks) ? nodeWorkload.finalNode : nodeWorkload;
  // 人员综合健康复用最终交付节点的累计工作与产能，避免全部剩余工作与最早已超期节点的 0h 产能错配。
  const target = workloadNode?.targetDate || fallbackTarget;
  const targetDateIncluded = workloadNode?.targetDateIncluded ?? fallbackTargetIncluded;
  const remaining = workloadNode?.required ?? fallbackRemaining;
  const availableDays = workloadNode?.availableDays
    ?? (target ? pmhWorkdaysToTarget(pmhToday(), target, targetDateIncluded) : 0);
  const capacity = workloadNode?.capacity ?? availableDays * PMH_DAILY_CAPACITY_HOURS;
  const loadRatio = workloadNode?.load
    ?? (capacity > 0 ? remaining / capacity : remaining > 0 ? Number.POSITIVE_INFINITY : 0);
  ...
  const confidence = actualTasks.length > 0
    ...
    : 0;
  let level = pmhHealthLevel(category.deviation, loadRatio, blocked, confidence);
  if (nodeWorkload?.level
    && pmhPressureLevelRank(nodeWorkload.level) < pmhPressureLevelRank(level)) {
    level = nodeWorkload.level;
  }
  ...
  return {
    ...
    gap: pmhRound(workloadNode?.gap ?? Math.max(remaining - capacity, 0)),
    loadRatio: Number.isFinite(loadRatio) ? pmhRound(loadRatio * 100) : PMH_OVERDUE_LOAD_VALUE,
    forecastDate: pmhForecastDate(
      remaining,
      PMH_DAILY_CAPACITY_HOURS,
      pmhEffectiveTargetDate(target, targetDateIncluded),
    ),
    targetDate: target,
    targetDateIncluded,
    blocked,
 ...
 */
function pmhMilestoneForecast(tasks, targetDate, totalTasks = tasks, targetDateIncluded = true) {
  ...
  const totalWorkload = new Map();
  for (const task of totalTasks) {
    const estimate = pmhTaskEstimate(task);
    if (estimate <= 0) continue;

    const assignees = pmhTaskWorkOwners(task);
    const owners = assignees.length > 0 ? assignees : [`未分配`];
    const sharedEstimate = estimate / owners.length;
    for (const owner of owners) {
      totalWorkload.set(owner, (totalWorkload.get(owner) ?? 0) + sharedEstimate);
    }
  }
  const totalDistribution = [...totalWorkload.entries()].map(([name, hours]) => ({
    name,
    hours: pmhRound(hours),
  })).sort((left, right) => right.hours - left.hours
    || left.name.localeCompare(right.name, `zh-CN`));
  const bottleneck = distribution[0] ?? { name: `无`, hours: 0, workdays: 0 };
  const totalHours = totalTasks.reduce((total, task) => total + pmhTaskEstimate(task), 0);
  const remaining = tasks.reduce((total, task) => total + pmhTaskRemaining(task), 0);
  const validTargetDate = pmhValidDate(targetDate);
  const effectiveTargetDate = pmhEffectiveTargetDate(validTargetDate, targetDateIncluded);
  const availableDays = effectiveTargetDate ? pmhWorkdaysBetween(pmhToday(), effectiveTargetDate) : 0;
  ...
  const forecastDate = remaining <= 0
    ? effectiveTargetDate || validTargetDate
    : pmhAddWorkdays(pmhToday(), bottleneck.workdays);

  return {
    totalHours: pmhRound(totalHours),
    totalDistribution,
    remaining: pmhRound(remaining),
    capacity: pmhRound(capacity),
    load: maximumLoad,
    ...
    availableDays,
    targetDateIncluded: targetDateIncluded !== false,
    effectiveTargetDate,
...
/**
 * 整体视角累计计算截至当前展示节点的交付批次。
 * 前序批次的未完成工作仍会占用后续节点前的产能，不能从预计日期和负载中排除。
 */
function pmhDeliveryMilestoneForecast(tasks, totalTasks, deliveryContext, phase, fallbackTargetDate) {
  if (!deliveryContext?.enabled) {
    return pmhMilestoneForecast(tasks, fallbackTargetDate, totalTasks);
  }
  if (deliveryContext.scopeId !== PM_DELIVERY_SCOPE_ALL) {
    const scopedBatch = deliveryContext.batchById.get(deliveryContext.scopeId);
    return pmhMilestoneForecast(
      tasks,
      fallbackTargetDate,
      totalTasks,
      pmDeliveryBoundaryIncluded(scopedBatch, phase === `testing` ? `testing` : `release`),
    );
  }
  ...
  if (targetBatches.length === 0) return pmhMilestoneForecast(tasks, fallbackTargetDate, totalTasks);

  const cumulativeBatches = deliveryContext.batches.filter((batch) => {
    const batchTargetDate = phase === `testing`
      ? pmhValidDate(batch.testingDate)
      : pmhValidDate(batch.releaseDate);
    return Boolean(batchTargetDate && batchTargetDate <= targetDate);
  });
  const cumulativeBatchIds = new Set(cumulativeBatches.map((batch) => String(batch.id)));
  const targetTasks = tasks.filter((task) => cumulativeBatchIds.has(
    String(deliveryContext.taskBatchIdByTaskId.get(String(task.id ?? ``)) ?? ``),
  ));
  const targetTotalTasks = totalTasks.filter((task) => cumulativeBatchIds.has(
    String(deliveryContext.taskBatchIdByTaskId.get(String(task.id ?? ``)) ?? ``),
  ));
  // 同日多批次规则不一致时取更保守的“不含当日”口径。
  const targetDateIncluded = targetBatches.every((batch) => pmDeliveryBoundaryIncluded(
    batch,
    phase === `testing` ? `testing` : `release`,
  ));
  const forecast = pmhMilestoneForecast(targetTasks, targetDate, targetTotalTasks, targetDateIncluded);
 ...
 */
function pmhDeliveryOverdueMilestones(unfinishedTasks, allTasks, deliveryContext) {
  ...
  for (const batch of deliveryContext.batches) {
    ...
    const allBatchTasks = allTasks.filter((task) => (
      deliveryContext.taskBatchIdByTaskId.get(String(task.id ?? ``)) === batch.id
    ));
    const developmentTasks = batchTasks.filter((task) => pmhTaskKind(task) === `development`);
    const allDevelopmentTasks = allBatchTasks.filter((task) => pmhTaskKind(task) === `development`);
    const testingDate = pmhValidDate(batch.testingDate);
    const releaseDate = pmhValidDate(batch.releaseDate);
    const testingDateIncluded = pmDeliveryBoundaryIncluded(batch, `testing`);
    const releaseDateIncluded = pmDeliveryBoundaryIncluded(batch, `release`);
    let phase = ``;
    let targetDate = ``;
    let targetDateIncluded = true;
    let milestoneTasks = [];
    let totalMilestoneTasks = [];

    if (releaseDate && pmhTargetDatePassed(releaseDate, releaseDateIncluded, today) && batchTasks.length > 0) {
      phase = `release`;
      targetDate = releaseDate;
      targetDateIncluded = releaseDateIncluded;
      milestoneTasks = batchTasks;
      totalMilestoneTasks = allBatchTasks;
    } else if (testingDate && pmhTargetDatePassed(testingDate, testingDateIncluded, today) && developmentTasks.length > 0) {
      phase = `testing`;
      targetDate = testingDate;
      targetDateIncluded = testingDateIncluded;
      milestoneTasks = developmentTasks;
      totalMilestoneTasks = allDevelopmentTasks;
    }
    if (!targetDate) continue;

    overdueMilestones.push({
      ...pmhMilestoneForecast(milestoneTasks, targetDate, totalMilestoneTasks, targetDateIncluded),
      batch,
      phase,
      targetDate,
    });
...
function pmhPressureTaskTarget(task, projectDates, deliveryContext) {
  const kind = pmhTaskKind(task);
  const batch = pmDeliveryTaskBatch(task, deliveryContext);
  const boundaryPhase = kind === `testing` ? `release` : `testing`;
  ...
  const targetDate = batchTarget || taskDue || fallbackTarget;
  const targetDateIncluded = batchTarget ? pmDeliveryBoundaryIncluded(batch, boundaryPhase) : true;
  return {
    targetDate,
    effectiveTargetDate: pmhEffectiveTargetDate(targetDate, targetDateIncluded),
    targetDateIncluded,
...
function pmhBuildPressure(workItems, unfinishedTasks, projectDates, deliveryContext) {
  ...
  const taskPressure = unfinishedTasks.map((task) => {
    ...
    const overdue = pmhTargetDatePassed(target.targetDate, target.targetDateIncluded);
    ...
    return {
      ...
      effectiveTargetDate: target.effectiveTargetDate,
      targetDateIncluded: target.targetDateIncluded,
      targetType: target.targetType,
      targetSource: target.targetSource,
      batchName: target.batchName,
      availableDays: target.targetDate
        ? pmhWorkdaysToTarget(pmhToday(), target.targetDate, target.targetDateIncluded)
        : 0,
      blocked,
      overdue,
      unestimated,
  ...
  const memberPressure = [...memberNames].map((name) => {
    ...
    const datedTasks = ownedTasks
      .filter((task) => task.targetDate && task.effectiveTargetDate)
      .sort((left, right) => left.effectiveTargetDate.localeCompare(right.effectiveTargetDate)
        || left.targetDate.localeCompare(right.targetDate)
        || right.remaining - left.remaining);
    const targetDates = [...new Set(datedTasks.map((task) => task.effectiveTargetDate))];
    let previousNode = null;
    const nodeRisks = targetDates.map((effectiveTargetDate) => {
      const nodeTasks = datedTasks.filter((task) => task.effectiveTargetDate <= effectiveTargetDate);
      const boundaryTasks = datedTasks.filter((task) => task.effectiveTargetDate === effectiveTargetDate);
      const required = nodeTasks.reduce((total, task) => total + shareFor(task), 0);
      const developmentRequired = nodeTasks
        .filter((task) => task.kind === `development`)
        .reduce((total, task) => total + shareFor(task), 0);
      const testingRequired = nodeTasks
        .filter((task) => task.kind === `testing`)
        .reduce((total, task) => total + shareFor(task), 0);
      const availableDays = pmhWorkdaysBetween(pmhToday(), effectiveTargetDate);
      ...
      const gap = Math.max(required - capacity, 0);
      const nodeRisk = {
        targetDate: boundaryTasks[0]?.targetDate ?? effectiveTargetDate,
        effectiveTargetDate,
        targetDateIncluded: boundaryTasks.every((task) => task.targetDateIncluded !== false),
        targetType: boundaryTasks[0]?.targetType ?? `交付`,
        required: pmhRound(required),
        developmentRequired: pmhRound(developmentRequired),
        testingRequired: pmhRound(testingRequired),
        availableDays,
        capacity: pmhRound(capacity),
        gap: pmhRound(gap),
        addedRequired: pmhRound(required - (previous?.required ?? 0)),
        addedCapacity: pmhRound(capacity - (previous?.capacity ?? 0)),
        carriedGap: previous?.gap ?? 0,
        gapDelta: pmhRound(gap - (previous?.gap ?? 0)),
        load,
        loadLabel: pmhPressureLoadLabel(load),
      };
      previousNode = nodeRisk;
      return nodeRisk;
    });
    const bottleneck = [...nodeRisks].sort((left, right) => right.gap - left.gap
      || (Number.isFinite(right.load) ? right.load : Number.MAX_SAFE_INTEGER)
        - (Number.isFinite(left.load) ? left.load : Number.MAX_SAFE_INTEGER)
      || left.effectiveTargetDate.localeCompare(right.effectiveTargetDate))[0] ?? null;
    const finalNode = nodeRisks[nodeRisks.length - 1] ?? null;
    const load = finalNode?.load ?? 0;
    const gap = finalNode?.gap ?? 0;
    const riskLoad = bottleneck?.load ?? load;
    const riskGap = bottleneck?.gap ?? gap;
    ...
    if (gap > 0) reasons.push(`最终节点累计缺口${gap}h`);
    else if (riskGap > 0) reasons.push(`${pmhFormatDate(bottleneck?.targetDate)}节点缺口${riskGap}h`);
    ...
    if (reasons.length === 0 && riskLoad > PMH_HEALTHY_LOAD) reasons.push(`节点可用产能接近满载`);
    ...
    return {
      ...
      load,
      loadLabel: pmhPressureLoadLabel(load),
      required: finalNode?.required ?? 0,
      capacity: finalNode?.capacity ?? 0,
      gap,
      targetDate: finalNode?.targetDate ?? ``,
      effectiveTargetDate: finalNode?.effectiveTargetDate ?? ``,
      targetDateIncluded: finalNode?.targetDateIncluded ?? true,
      targetType: finalNode?.targetType ?? `交付`,
      availableDays: finalNode?.availableDays ?? 0,
      forecastDate: pmhForecastDate(totalRemaining, PMH_DAILY_CAPACITY_HOURS, finalNode?.targetDate),
      riskGap,
      riskLoad,
    ...
    const rightLoad = Number.isFinite(right.riskLoad) ? right.riskLoad : Number.MAX_SAFE_INTEGER;
    const leftLoad = Number.isFinite(left.riskLoad) ? left.riskLoad : Number.MAX_SAFE_INTEGER;
    return pmhPressureLevelRank(left.level) - pmhPressureLevelRank(right.level)
      || right.riskGap - left.riskGap
      || rightLoad - leftLoad
      || right.remaining - left.remaining
      || left.name.localeCompare(right.name, `zh-CN`);
  });
  ...
  for (const task of taskPressure) {
    const ownerNodeRisks = task.owners.map((owner) => {
      ...
      const nodeRisk = member?.nodeRisks.find((node) => (
        node.effectiveTargetDate === task.effectiveTargetDate
      )) ?? null;
  ...
  for (const task of taskPressure) {
    ...
    if (!requirement) {
      requirement = {
        ...
        effectiveTargetDate: task.effectiveTargetDate,
        targetDateIncluded: task.targetDateIncluded,
    ...
    if (!requirement.effectiveTargetDate
      || (task.effectiveTargetDate && task.effectiveTargetDate < requirement.effectiveTargetDate)) {
      requirement.targetDate = task.targetDate;
      requirement.effectiveTargetDate = task.effectiveTargetDate;
      requirement.targetDateIncluded = task.targetDateIncluded;
  ...
  const requirementPressure = [...requirementMap.values()].map((requirement) => ({
    ...
    availableDays: requirement.effectiveTargetDate
      ? pmhWorkdaysBetween(pmhToday(), requirement.effectiveTargetDate)
      : 0,
    ...
    || (left.effectiveTargetDate || `9999-12-31`).localeCompare(right.effectiveTargetDate || `9999-12-31`)
    || right.remaining - left.remaining
    || left.title.localeCompare(right.title, `zh-CN`));
...
function pmhBuildProjectHealth(
  ...
  const pressureMemberByName = new Map(pressure.members.map((member) => [member.name, member]));
  const members = [...new Set(actualTasks.flatMap(pmhTaskAssignees))]
    .map((name) => pmhMemberHealth(
      name,
      actualTasks,
      projectDates,
      deliveryContext,
      pressureMemberByName.get(name) ?? null,
    ))
    .sort((left, right) => {
  ...
  const testingForecast = pmhDeliveryMilestoneForecast(
    preTestingTasks,
    actualTasks.filter((task) => pmhTaskKind(task) === `development`),
    deliveryContext,
    `testing`,
  ...
  const overdueMilestones = pmhDeliveryOverdueMilestones(unfinishedTasks, actualTasks, deliveryContext);
  ...
  return {
    ...
    pressure,
    testing: {
      totalHours: testingForecast.totalHours,
      totalDistribution: testingForecast.totalDistribution,
      remainingDistribution: testingForecast.distribution,
      ...
      targetDateIncluded: testingForecast.targetDateIncluded,
      effectiveTargetDate: testingForecast.effectiveTargetDate,
    },
    completion: {
      totalHours: completionForecast.totalHours,
      totalDistribution: completionForecast.totalDistribution,
      remainingDistribution: completionForecast.distribution,
      remaining: pmhRound(completionRemaining),
      capacity: completionForecast.capacity,
      loadRatio: completionForecast.loadRatio,
      forecastDate: completionForecast.forecastDate,
      targetDateIncluded: completionForecast.targetDateIncluded,
      effectiveTargetDate: completionForecast.effectiveTargetDate,
    },
    calculation: {
...
function pmhHealthCalculationDescriptions(health) {
  ...
  const comparisonText = (forecastValue, targetValue, targetDateIncluded = true) => {
    const forecastDate = pmhValidDate(forecastValue);
    const targetDate = pmhEffectiveTargetDate(targetValue, targetDateIncluded);
  ...
  return {
    ...
    testingLoad: health.testing.remaining <= 0
      ...
      : health.calculation.testingAvailableDays > 0
        ? `瓶颈人员 ${health.calculation.testingBottleneck.name}：${health.calculation.testingBottleneck.hours}h ÷（${health.calculation.testingAvailableDays}工作日 × ${PMH_DAILY_CAPACITY_HOURS}h）= ${pmhLoadLabel(health.testing.loadRatio)}（提测日${pmDeliveryBoundaryLabel(health.testing.targetDateIncluded)}）`
    ...
    testingForecast: health.testing.remaining <= 0 && health.dates.testing
      ? `目标提测时间 ${pmhFormatDate(health.dates.testing)}（${pmDeliveryBoundaryLabel(health.testing.targetDateIncluded)}）；对比结果：${comparisonText(health.testing.forecastDate, health.dates.testing, health.testing.targetDateIncluded)}（按工作日计算）`
      : `瓶颈人员 ${health.calculation.testingBottleneck.name} 剩余 ${health.calculation.testingBottleneck.hours}h ÷ ${PMH_DAILY_CAPACITY_HOURS}h/工作日 = ${health.calculation.testingWorkdays}个工作日；目标提测时间 ${pmhFormatDate(health.dates.testing)}（${pmDeliveryBoundaryLabel(health.testing.targetDateIncluded)}）；对比结果：${comparisonText(health.testing.forecastDate, health.dates.testing, health.testing.targetDateIncluded)}`,
    completionForecast: health.completion.remaining <= 0 && health.dates.completion
      ? `目标上线时间 ${pmhFormatDate(health.dates.completion)}（${pmDeliveryBoundaryLabel(health.completion.targetDateIncluded)}）；对比结果：${comparisonText(health.completion.forecastDate, health.dates.completion, health.completion.targetDateIncluded)}（按工作日计算）`
      : `瓶颈人员 ${health.calculation.completionBottleneck.name} 剩余 ${health.calculation.completionBottleneck.hours}h ÷ ${PMH_DAILY_CAPACITY_HOURS}h/工作日 = ${health.calculation.completionWorkdays}个工作日；目标上线时间 ${pmhFormatDate(health.dates.completion)}（${pmDeliveryBoundaryLabel(health.completion.targetDateIncluded)}）；对比结果：${comparisonText(health.completion.forecastDate, health.dates.completion, health.completion.targetDateIncluded)}`,
  };
}
...
function pmOpenIterationMemberHealth(view, member, health) {
  ...
  modal.onOpen = () => {
    ...
    const metrics = [
      ...
      [`累计缺口`, pmIterationFormatHours(member.gap ?? 0)],
      [`负载率`, pmhLoadLabel(member.loadRatio)],
      [`产能口径`, member.targetDate
        ? `截至${pmFormatDate(member.targetDate)}交付节点（${pmDeliveryBoundaryLabel(member.targetDateIncluded)}）`
        : `节点时间未维护`],
      ...
      [`总任务`, member.total],
      [`总工时`, pmIterationFormatHours(member.tasks.reduce((total, task) => total + pmhTaskEstimate(task), 0))],
    ];
...
function pmRenderIterationMembers(container, view, health) {
  ...
  for (const { member, bugStats, combinedLevel, combinedLabel } of visibleRows) {
    ...
    const pressureMember = health.pressure?.members.find((item) => item.name === member.name) ?? null;
    ...
    if ((pressureMember?.overdue ?? 0) > 0) healthReasons.push(`已超期任务 ${pressureMember.overdue}个`);
    ...
    workload.createSpan({
      text: `负载 ${pmhLoadLabel(member.loadRatio)}${member.gap > 0 ? ` · 缺口 ${pmIterationFormatHours(member.gap)}` : ``}`,
      cls: `is-${loadTone}`,
    });
    const workloadDescription = `${member.targetDate
      ? `截至${pmFormatDate(member.targetDate)}交付节点（${pmDeliveryBoundaryLabel(member.targetDateIncluded)}），`
      : ``}`
      + `累计工作 ${pmIterationFormatHours(member.remaining)}，可用产能 ${pmIterationFormatHours(member.capacity)}`
      + `，负载率 ${pmhLoadLabel(member.loadRatio)}${member.gap > 0 ? `，累计缺口 ${pmIterationFormatHours(member.gap)}` : ``}`;
    workload.setAttribute(`aria-label`, `${workloadDescription}，点击查看节点明细`);
    workload.setAttribute(`title`, `${workloadDescription}，点击查看节点明细`);
    if (pressureMember) {
      workload.addClass(`is-action`);
      workload.addEventListener(`click`, (event) => {
        event.stopPropagation();
        pmPressureOpenMemberRiskDetail(view, health.pressure, pressureMember, `overview`, workload);
      });
    }
...
function pmRenderIterationHealthOverview(container, view, health) {
  ...
  const metrics = [
    ...
    [`预计提测时间`, health.dates.testing ? pmFormatDate(health.testing.forecastDate) : `--`, ``, {
      ...
      targetDateIncluded: health.testing.targetDateIncluded,
    ...
    [`预计结束时间`, health.dates.completion ? pmFormatDate(health.completion.forecastDate) : `--`, ``, {
      ...
      targetDateIncluded: health.completion.targetDateIncluded,
  ...
  for (const [label, value, tone, timing, calculation, hint, metricId] of metrics) {
    ...
    const targetDate = pmhEffectiveTargetDate(timing.targetDate, timing.targetDateIncluded);
...
function pmhMilestoneBatches(health, phase, targetValue) {
  if (!health.delivery?.enabled) return [];

  const batches = Array.isArray(health.delivery.batches) ? health.delivery.batches : [];
  if (health.delivery.scopeId && health.delivery.scopeId !== PM_DELIVERY_SCOPE_ALL) {
    return batches.filter((batch) => String(batch.id) === String(health.delivery.scopeId));
  }

  const targetDate = pmhValidDate(targetValue);
  if (!targetDate) return [];
  const dateField = phase === `release` ? `releaseDate` : `testingDate`;
  return batches.filter((batch) => pmhValidDate(batch[dateField]) === targetDate);
}

function pmhMilestoneLoadDisplay(value) {
  return Number(value) === PMH_OVERDUE_LOAD_VALUE ? `—` : `${value}%`;
}

function pmhMilestoneHoursBreakdown(distribution) {
  if (!Array.isArray(distribution) || distribution.length === 0) return `暂无人员工时`;
  return distribution
    .filter((member) => pmhNumber(member.hours) > 0)
    .map((member) => `${member.name} ${pmIterationFormatHours(member.hours)}`)
    .join(` + `) || `暂无人员工时`;
}

function pmRenderIterationMilestoneHours(row, label, hours, distribution) {
  const formattedHours = pmIterationFormatHours(hours);
  const breakdown = pmhMilestoneHoursBreakdown(distribution);
  return row.createSpan({
    text: formattedHours,
    cls: `pm-iteration-milestone-hours`,
    attr: {
      title: `${label} ${formattedHours}\n${breakdown}`,
      tabindex: `0`,
    },
  });
}

function pmhMilestoneStatus(milestone, targetValue) {
  const targetDate = pmhValidDate(targetValue);
  const targetDateIncluded = milestone.targetDateIncluded !== false;
  const effectiveTargetDate = milestone.effectiveTargetDate
    || pmhEffectiveTargetDate(targetDate, targetDateIncluded);
  const forecastDate = pmhValidDate(milestone.forecastDate);
  const remaining = pmhNumber(milestone.remaining);
  const loadRatio = Number(milestone.loadRatio);
  if (!targetDate) {
    return {
      label: `日期未维护`,
      tone: `muted`,
      reason: `该节点没有可用的目标日期，暂时无法判断是否能按期完成。`,
    };
  }

  const today = pmhToday();
  if (remaining > 0 && effectiveTargetDate < today) {
    const overdueDays = Math.abs(pmhWorkdayDateDelta(today, effectiveTargetDate));
    return {
      label: overdueDays > 0 ? `已超期 ${overdueDays} 个工作日` : `已超期`,
      tone: `danger`,
      reason: `目标日期${pmDeliveryBoundaryLabel(targetDateIncluded)}，可用工作截止日已过，但节点仍有 ${pmIterationFormatHours(remaining)} 未完成工作。`,
    };
  }

  const forecastDelta = forecastDate ? pmhWorkdayDateDelta(forecastDate, effectiveTargetDate) : 0;
  if (remaining > 0 && forecastDelta < 0) {
    return {
      label: `预计延期 ${Math.abs(forecastDelta)} 个工作日`,
      tone: `warning`,
      reason: `预计完成时间晚于目标日期。`,
    };
  }

  if (remaining > 0 && (loadRatio > 100 || loadRatio === PMH_OVERDUE_LOAD_VALUE)) {
    return {
      label: `超负荷`,
      tone: `warning`,
      reason: `当前剩余工作已经超过目标日期前的可用产能。`,
    };
  }
  ...
  return {
    label: `正常`,
    tone: `healthy`,
    reason: `按当前剩余工作和可用产能，预计可以按期完成。`,
  };
}

function pmRenderIterationMilestoneRow(section, item) {
  const status = pmhMilestoneStatus(item.milestone, item.targetDate);
  const row = section.createDiv(`pm-iteration-milestone-row is-status-${status.tone}`);
  row.createEl(`strong`, {
    text: item.label,
    attr: { title: item.label },
  });
  const loadRatio = Number(item.milestone.loadRatio);
  const loadTone = loadRatio === PMH_OVERDUE_LOAD_VALUE
    ? `is-muted`
    : loadRatio > 100
      ? `is-danger`
      : loadRatio > 85
  ...
  const statusButton = row.createEl(`button`, {
    text: status.label,
    cls: `pm-iteration-milestone-status is-${status.tone}`,
    attr: {
      type: `button`,
      title: `${status.label}，点击查看判断依据`,
      'aria-expanded': `false`,
    },
  });
  row.createSpan({
    text: `${pmFormatDate(item.targetDate)} · ${pmDeliveryBoundaryLabel(item.milestone.targetDateIncluded)}`,
  });
  row.createSpan({ text: pmFormatDate(item.milestone.forecastDate) });
  pmRenderIterationMilestoneHours(
    row,
    `总工时`,
    item.milestone.totalHours,
    item.milestone.totalDistribution,
  );
  pmRenderIterationMilestoneHours(
    row,
    `剩余工时`,
    item.milestone.remaining,
    item.milestone.remainingDistribution ?? item.milestone.distribution,
  );
  row.createSpan({ text: pmIterationFormatHours(item.milestone.capacity) });
  row.createSpan({
    text: pmhMilestoneLoadDisplay(loadRatio),
    cls: loadTone,
  });

  const details = row.createDiv({
    cls: `pm-iteration-milestone-details`,
    attr: { hidden: ``, role: `status` },
  });
  details.createSpan({
    text: `目标日期：${pmFormatDate(item.targetDate)}（${pmDeliveryBoundaryLabel(item.milestone.targetDateIncluded)}）`,
  });
  if (item.milestone.targetDateIncluded === false) {
    details.createSpan({ text: `有效工作截止：${pmFormatDate(item.milestone.effectiveTargetDate)}` });
  }
  details.createSpan({ text: `总工时：${pmIterationFormatHours(item.milestone.totalHours)}` });
  details.createSpan({ text: `剩余工时：${pmIterationFormatHours(item.milestone.remaining)}` });
  details.createSpan({ text: `剩余产能：${pmIterationFormatHours(item.milestone.capacity)}` });
  details.createSpan({ text: `负载率：${pmhMilestoneLoadDisplay(loadRatio)}` });
  details.createSpan({ text: `预计完成：${pmFormatDate(item.milestone.forecastDate)}` });
  details.createSpan({ text: `原因：${status.reason}`, cls: `pm-iteration-milestone-reason` });

  statusButton.addEventListener(`click`, () => {
    const expanded = statusButton.getAttribute(`aria-expanded`) === `true`;
    statusButton.setAttribute(`aria-expanded`, String(!expanded));
...
function pmRenderIterationMilestones(container, health) {
  ...
  for (const label of [`节点`, `状态`, `目标日期`, `预计完成`, `总工时`, `剩余工时`, `剩余产能`, `负载率`]) header.createSpan({ text: label });
  ...
  for (const [phase, phaseLabel, fallbackLabel, targetDate, milestone] of [
    [`testing`, `提测节点`, testingLabel, health.dates.testing, health.testing],
    [`release`, `上线节点`, completionLabel, health.dates.completion, health.completion],
  ]) {
    const batches = pmhMilestoneBatches(health, phase, targetDate);
    const batchNames = batches.map((batch) => String(batch.name ?? ``).trim()).filter(Boolean);
    const label = batchNames.length > 0 ? `${batchNames.join(`、`)} · ${phaseLabel}` : fallbackLabel;
    for (const batch of batches) {
      renderedMilestones.add(`${batch.id}:${phase}:${pmhValidDate(targetDate)}`);
    }
    pmRenderIterationMilestoneRow(section, { label, targetDate, milestone });
  }

  for (const milestone of health.overdueMilestones ?? []) {
    const phaseLabel = milestone.phase === `release` ? `上线` : `提测`;
    const milestoneKey = `${milestone.batch.id}:${milestone.phase}:${pmhValidDate(milestone.targetDate)}`;
    if (renderedMilestones.has(milestoneKey)) continue;
    pmRenderIterationMilestoneRow(section, {
      label: `${milestone.batch.name} · ${phaseLabel}节点`,
...
function pmPressureTimeLabel(item) {
  ...
  if (item.overdue > 0 || item.overdue === true
    || pmhTargetDatePassed(item.targetDate, item.targetDateIncluded)) {
    return `${pmhFormatDate(item.targetDate)} ${item.targetType ?? `交付`} · ${pmDeliveryBoundaryLabel(item.targetDateIncluded)} · 已超期`;
  }
  if (item.availableDays === 0) return `${pmhFormatDate(item.targetDate)} ${item.targetType ?? `交付`} · ${pmDeliveryBoundaryLabel(item.targetDateIncluded)} · 今天`;
  return `${pmhFormatDate(item.targetDate)} ${item.targetType ?? `交付`} · ${pmDeliveryBoundaryLabel(item.targetDateIncluded)} · 剩余${item.availableDays}个工作日`;
}
...
function pmPressureMemberRiskGroups(member, tasks) {
  ...
  const nodeTasks = member.effectiveTargetDate
    ? tasks.filter((task) => task.effectiveTargetDate
      && task.effectiveTargetDate <= member.effectiveTargetDate)
    : [];
  if (member.gap > 0) {
    groups.push({
      id: `gap`,
      label: member.finalNode?.effectiveTargetDate === member.effectiveTargetDate
        ? `最终节点累计缺口${member.gap}h`
        : `${pmhFormatDate(member.targetDate)}节点累计缺口${member.gap}h`,
      taskBadge: `参与缺口`,
      tone: `danger`,
      tasks: nodeTasks,
      basis: `节点前剩余工作超过可用产能`,
      aggregate: true,
      node: member.nodeRisks?.find((nodeRisk) => (
        nodeRisk.effectiveTargetDate === member.effectiveTargetDate
      )) ?? null,
    });
    if (member.finalNode?.effectiveTargetDate === member.effectiveTargetDate
      && (member.finalNode?.carriedGap ?? 0) > 0
      && (member.finalNode?.gapDelta ?? 0) > 0) {
      groups.push({
        id: `incremental-gap`,
        label: `${member.finalNode.targetType}节点新增缺口${member.finalNode.gapDelta}h`,
        taskBadge: `本节点新增`,
        tone: `danger`,
        tasks: tasks.filter((task) => task.effectiveTargetDate === member.finalNode.effectiveTargetDate),
        basis: `上一节点缺口结转后，本节点新增工作仍超过新增可用产能`,
        aggregate: true,
        node: member.finalNode,
      });
    }
  } else if ((member.riskGap ?? 0) > 0 && member.bottleneckNode) {
    groups.push({
      id: `bottleneck-gap`,
      label: `${pmhFormatDate(member.bottleneckNode.targetDate)}节点缺口${member.riskGap}h`,
      taskBadge: `参与节点缺口`,
      tone: `danger`,
      tasks: tasks.filter((task) => task.effectiveTargetDate
        && task.effectiveTargetDate <= member.bottleneckNode.effectiveTargetDate),
      basis: `中间节点产能不足，但最终节点前仍有机会追回`,
      aggregate: true,
      node: member.bottleneckNode,
    });
  }
  ...
  if (member.gap <= 0 && (member.riskGap ?? 0) <= 0 && member.riskLoad > PMH_HEALTHY_LOAD) {
    const loadNode = member.bottleneckNode ?? member.finalNode;
    groups.push({
      id: `load`,
      label: `${pmhFormatDate(loadNode?.targetDate)}节点产能接近满载`,
      taskBadge: `负载构成`,
      tone: `warning`,
      tasks: loadNode?.effectiveTargetDate
        ? tasks.filter((task) => task.effectiveTargetDate
          && task.effectiveTargetDate <= loadNode.effectiveTargetDate)
        : nodeTasks,
      basis: `节点负载已超过健康阈值`,
      aggregate: true,
      node: loadNode ?? null,
...
function pmPressureOpenMemberRiskDetail(view, pressure, member, initialRiskId, anchor) {
  ...
  if (initialRiskId === `overview` || groups.length === 0) {
    const nodeTasks = member.effectiveTargetDate
      ? memberTasks.filter((task) => task.effectiveTargetDate
        && task.effectiveTargetDate <= member.effectiveTargetDate)
      : memberTasks;
    groups.unshift({
      id: `overview`,
      label: `节点产能`,
      taskBadge: `工作构成`,
      tone: member.gap > 0 ? `danger` : member.load > PMH_HEALTHY_LOAD ? `warning` : `healthy`,
      tasks: nodeTasks,
      basis: `节点前累计工作与可用产能`,
      aggregate: true,
    });
  ...
  headerCopy.createEl(`strong`, { text: `${member.name} · 节点产能明细` });
  const uniqueTaskIds = new Set(groups.flatMap((group) => group.tasks.map((task) => task.id)));
  headerCopy.createSpan({
    text: `${groups.length === 1 ? groups[0].label : `${groups.length}类风险`}`
      + ` · 预计完成${pmhFormatDate(member.forecastDate)} · 涉及${uniqueTaskIds.size}项任务`,
  });
  ...
  const renderContent = () => {
    ...
    if (selectedGroup?.aggregate) {
      const calculationMember = selectedGroup.node ? { ...member, ...selectedGroup.node } : member;
      const calculation = content.createDiv(`pm-project-pressure-risk-calculation`);
      calculation.createDiv({
        text: `${pmhFormatDate(calculationMember.targetDate)} ${calculationMember.targetType ?? `交付`}节点 · ${pmDeliveryBoundaryLabel(calculationMember.targetDateIncluded)}`,
        cls: `pm-project-pressure-risk-calculation-title`,
      });
      calculation.createEl(`strong`, {
        text: !calculationMember.targetDate
          ? `累计工作 ${calculationMember.required}h · 产能待确认`
          : calculationMember.gap > 0
          ? `${calculationMember.required}h − ${calculationMember.capacity}h = 累计缺口 ${calculationMember.gap}h`
          : `${calculationMember.required}h / ${calculationMember.capacity}h · 负载 ${calculationMember.loadLabel}`,
        cls: `pm-project-pressure-risk-calculation-summary${calculationMember.targetDate && calculationMember.gap > 0 ? ` is-danger` : ``}`,
      });
      calculation.createDiv({
        text: !calculationMember.targetDate
          ? `交付时间未维护，暂不使用 0h 产能推导负载或缺口。`
          : calculationMember.gap > 0 && calculationMember.carriedGap > 0
          ? `上一节点遗留 ${calculationMember.carriedGap}h + 本节点新增工作 ${calculationMember.addedRequired}h − 新增产能 ${calculationMember.addedCapacity}h = 累计缺口 ${calculationMember.gap}h。`
          : calculationMember.gap > 0
            ? `节点前累计工作 − 可用产能；${calculationMember.availableDays}个工作日 × ${PMH_DAILY_CAPACITY_HOURS}h/天。`
          : `剩余缓冲${pmhRound(Math.max(calculationMember.capacity - calculationMember.required, 0))}h；${calculationMember.availableDays}个工作日 × ${PMH_DAILY_CAPACITY_HOURS}h/天。`,
        cls: `pm-project-pressure-risk-calculation-formula`,
      });

      if ((member.nodeRisks?.length ?? 0) > 1) {
        const timeline = calculation.createDiv(`pm-project-pressure-risk-timeline`);
        for (const nodeRisk of member.nodeRisks) {
          const selected = nodeRisk.effectiveTargetDate === calculationMember.effectiveTargetDate;
          const row = timeline.createDiv(`pm-project-pressure-risk-timeline-row${selected ? ` is-selected` : ``}`);
          const nodeCopy = row.createDiv();
          nodeCopy.createEl(`strong`, {
            text: `${pmhFormatDate(nodeRisk.targetDate)} ${nodeRisk.targetType}节点 · ${pmDeliveryBoundaryLabel(nodeRisk.targetDateIncluded)}`,
          });
          nodeCopy.createSpan({
            text: pmhTargetDatePassed(nodeRisk.targetDate, nodeRisk.targetDateIncluded)
              ? `已超期`
              : `新增工作${nodeRisk.addedRequired}h · 新增产能${nodeRisk.addedCapacity}h`,
          });
          row.createEl(`strong`, {
            text: nodeRisk.gap > 0 ? `累计缺口${nodeRisk.gap}h` : `缓冲${pmhRound(nodeRisk.capacity - nodeRisk.required)}h`,
            cls: nodeRisk.gap > 0 ? `is-danger` : `is-healthy`,
          });
        }
      }
    }
...
function pmRenderProjectPressure(container, view, health) {
  ...
  for (const member of visibleMembers.slice(0, activeSummaryFilter ? visibleMembers.length : 8)) {
    ...
    const healthMember = health.members.find((item) => item.name === member.name) ?? null;
    if (healthMember) {
      identity.addClass(`is-action`);
      identity.setAttrs({ role: `button`, tabindex: `0`, title: `查看人员综合健康详情` });
      const openHealthDetail = (event) => {
        event.stopPropagation();
        pmOpenIterationMemberHealth(view, healthMember, health);
      };
      identity.addEventListener(`click`, openHealthDetail);
      identity.addEventListener(`keydown`, (event) => {
        if (![`Enter`, ` `].includes(event.key)) return;
        event.preventDefault();
        openHealthDetail(event);
      });
    }
    cardHeader.createSpan({ text: member.label, cls: `pm-project-pressure-badge` });

    // 默认展示最终交付口径，并允许在卡片内切换查看每个节点。
    const nodeChoices = member.nodeRisks ?? [];
    let selectedNode = member.finalNode ?? nodeChoices[nodeChoices.length - 1] ?? {
      targetDate: ``,
      targetType: `交付`,
      required: member.remaining,
      developmentRequired: member.developmentRemaining,
      testingRequired: member.testingRemaining,
      availableDays: 0,
      capacity: 0,
      gap: 0,
      load: 0,
    ...
    let selectedNodeId = `overall`;
    const nodeSwitch = card.createDiv(`pm-project-pressure-member-node-switch`);
    const nodeButtons = new Map();
    const nodeOptions = selectedNode
      ? [
        [`overall`, `总体 · ${selectedNode.calculable === false ? `信息不足` : selectedNode.gap > 0 ? `缺${selectedNode.gap}h` : `可承接`}`, selectedNode],
        ...nodeChoices.map((nodeRisk) => [
          nodeRisk.effectiveTargetDate,
          `${pmhFormatDate(nodeRisk.targetDate)} ${nodeRisk.targetType} · ${pmDeliveryBoundaryLabel(nodeRisk.targetDateIncluded)} · ${nodeRisk.gap > 0 ? `累计缺${nodeRisk.gap}h` : `可承接`}`,
          nodeRisk,
        ]),
      ]
      : [];

    // 将节点、负载和工作构成放在同一视觉层级，所有数字始终对应同一节点范围。
    const focus = card.createDiv(`pm-project-pressure-member-focus`);
    const node = focus.createDiv(`pm-project-pressure-member-node`);
    const nodeLabel = node.createSpan();
    const nodeDate = node.createEl(`strong`);
    const nodeTime = node.createSpan({ cls: `pm-project-pressure-member-node-time` });
    ...
    const workloadValue = workloadCopy.createEl(`strong`);
    const workloadLoad = workloadHeader.createSpan({ cls: `pm-project-pressure-member-load` });
    const workloadTrack = workload.createDiv(`pm-project-pressure-member-load-track`);
    const workloadFill = workloadTrack.createSpan(`pm-project-pressure-member-load-fill`);
    const workloadGap = workload.createEl(`button`, {
      cls: `pm-project-pressure-member-gap`,
      attr: { type: `button`, 'aria-haspopup': `dialog` },
    });
    workloadGap.addEventListener(`click`, (event) => {
      event.stopPropagation();
      if (!selectedNode) return;
      pmPressureOpenMemberRiskDetail(view, pressure, {
        ...member,
        ...selectedNode,
        finalNode: member.finalNode,
        nodeRisks: member.nodeRisks,
        forecastDate: member.forecastDate,
      }, selectedNode.gap > 0 ? `gap` : `overview`, workloadGap);
    });

    const stage = focus.createDiv(`pm-project-pressure-member-stage`);
    stage.createSpan({ text: `工作构成` });
    const developmentValue = stage.createEl(`strong`);
    const testingValue = stage.createSpan();

    const renderSelectedNode = () => {
      if (!selectedNode) return;
      const overall = selectedNodeId === `overall`;
      const overdueNode = selectedNode.targetDate
        && pmhTargetDatePassed(selectedNode.targetDate, selectedNode.targetDateIncluded);
      nodeLabel.setText(`${overall ? `最终` : `所选`}${selectedNode.targetType ?? `交付`}节点`);
      nodeDate.setText(pmhFormatDate(selectedNode.targetDate));
      const nodeTimeText = !selectedNode.targetDate
        ? `时间未维护`
        : overdueNode
          ? `已超期`
          : selectedNode.availableDays === 0 ? `今天` : `剩余${selectedNode.availableDays}个工作日`;
      nodeTime.setText(selectedNode.targetDate
        ? `${nodeTimeText} · ${pmDeliveryBoundaryLabel(selectedNode.targetDateIncluded)}`
        : nodeTimeText);
      workloadValue.setText(selectedNode.calculable === false
        ? `累计 ${selectedNode.required}h / 可用 --`
        : `累计 ${selectedNode.required}h / 可用 ${selectedNode.capacity}h`);
      workloadLoad.setText(selectedNode.loadLabel ?? pmhPressureLoadLabel(selectedNode.load));
      workloadFill.style.width = `${pmhClamp(Number.isFinite(selectedNode.load) ? selectedNode.load : 1) * 100}%`;
      workloadGap.setText(selectedNode.calculable === false
        ? `交付时间未维护，暂不计算产能 ›`
        : selectedNode.gap > 0
          ? `${overall ? `最终节点` : `所选节点`}累计缺口 ${selectedNode.gap}h ›`
          : `剩余 ${pmhRound(Math.max(selectedNode.capacity - selectedNode.required, 0))}h 产能缓冲 ›`);
      workloadGap.toggleClass(`is-danger`, selectedNode.gap > 0);
      developmentValue.setText(`开发 ${selectedNode.developmentRequired ?? member.developmentRemaining}h`);
      testingValue.setText(`测试 ${selectedNode.testingRequired ?? member.testingRemaining}h`);
      for (const [id, button] of nodeButtons) {
        const activeNode = id === selectedNodeId;
        button.toggleClass(`is-active`, activeNode);
        button.setAttribute(`aria-pressed`, String(activeNode));
      }
    };

    for (const [id, label, nodeRisk] of nodeOptions) {
      const button = nodeSwitch.createEl(`button`, {
        text: label,
        attr: { type: `button`, 'aria-pressed': String(id === selectedNodeId) },
      });
      button.addEventListener(`click`, (event) => {
        event.stopPropagation();
        selectedNodeId = id;
        selectedNode = nodeRisk;
        renderSelectedNode();
      });
      nodeButtons.set(id, button);
    }
    if (nodeOptions.length <= 1) nodeSwitch.remove();
    renderSelectedNode();

    const assessment = card.createDiv(`pm-project-pressure-member-assessment`);
    assessment.createSpan({ text: `风险汇总` });
    ...
    if (memberRiskGroups.length === 0) {
      ...
      for (const group of memberRiskGroups.slice(0, 3)) {
      ...
      if (memberRiskGroups.length > 3) {
        const more = riskChips.createEl(`button`, {
          text: `+${memberRiskGroups.length - 3}`,
    ...
    if (member.primaryTaskInfo) {
      primaryTaskCopy.addClass(`is-action`);
      primaryTaskCopy.setAttrs({ role: `button`, tabindex: `0`, title: `打开任务详情` });
      primaryTaskCopy.createSpan({
        text: `${pmPressureTimeLabel(member.primaryTaskInfo)} · 剩余${member.primaryTaskInfo.remaining}h`,
      });
      const openPrimaryTask = (event) => {
        event.stopPropagation();
        pmPressureOpenTaskDetail(view, member.primaryTaskInfo);
      };
      primaryTaskCopy.addEventListener(`click`, openPrimaryTask);
      primaryTaskCopy.addEventListener(`keydown`, (event) => {
        if (![`Enter`, ` `].includes(event.key)) return;
        event.preventDefault();
        openPrimaryTask(event);
      });
    }
...
function pmDeliveryCreateBoundaryDateField(
  container,
  label,
  value,
  included,
  ...
  const field = container.createDiv(`pm-delivery-field pm-delivery-boundary-field`);
  field.createEl(`label`, { text: label });
  const input = field.createEl(`input`, { type: `date` });
  input.value = value;
  input.addEventListener(`input`, () => onDateInput(input.value));

  const toggle = field.createEl(`label`, { cls: `pm-delivery-boundary-toggle` });
  const checkbox = toggle.createEl(`input`, { type: `checkbox` });
  checkbox.checked = included !== false;
  toggle.createSpan({ text: includeLabel });
  const hint = toggle.createSpan({
    text: checkbox.checked ? `当日计入可用工期` : `可用工期截止到前一工作日`,
    cls: `pm-delivery-boundary-toggle-hint`,
  });
  checkbox.addEventListener(`change`, () => {
    hint.setText(checkbox.checked ? `当日计入可用工期` : `可用工期截止到前一工作日`);
    onIncludedChange(checkbox.checked);
  });
...
function pmOpenDeliveryDrawer(view, statuses, manualDates) {
  ...
  const renderDrawer = () => {
    ...
    headingCopy.createEl(`p`, { text: `提测、上线时间及当日是否计入工期，将统一参与应完成度、产能、负载、缺口和风险计算。` });
    ...
    for (const batch of draftPlan.batches) {
      ...
      selectButton.createSpan({
        text: `提测 ${pmDeliveryFormatDate(batch.testingDate)} · 开发${pmDeliveryBoundaryLabel(batch.includeTestingDate)}`,
      });
      selectButton.createSpan({
        text: `上线 ${pmDeliveryFormatDate(batch.releaseDate)} · 测试${pmDeliveryBoundaryLabel(batch.includeReleaseDate)}`,
      });
    ...
    const refreshImpact = () => {
      ...
      for (const [label, before, after] of [
        ...
        [`提测可用产能`, pmIterationFormatHours(currentSummary.health.testing.capacity), pmIterationFormatHours(nextSummary.health.testing.capacity)],
        [`上线可用产能`, pmIterationFormatHours(currentSummary.health.completion.capacity), pmIterationFormatHours(nextSummary.health.completion.capacity)],
        [`提测负载`, pmhLoadLabel(currentSummary.health.testing.loadRatio), pmhLoadLabel(nextSummary.health.testing.loadRatio)],
        [`上线负载`, pmhLoadLabel(currentSummary.health.completion.loadRatio), pmhLoadLabel(nextSummary.health.completion.loadRatio)],
      ]) {
    ...
    pmDeliveryCreateBoundaryDateField(
      fields,
      `提测时间`,
      batch.testingDate,
      batch.includeTestingDate,
      `开发工期包含提测日`,
      (value) => updateBatch(`testingDate`, value),
      (value) => updateBatch(`includeTestingDate`, value),
    );
    pmDeliveryCreateBoundaryDateField(
      fields,
      `上线时间`,
      batch.releaseDate,
      batch.includeReleaseDate,
      `测试工期包含上线日`,
      (value) => updateBatch(`releaseDate`, value),
      (value) => updateBatch(`includeReleaseDate`, value),
    );
    ...
    if (batchHistory.length > 0) {
      ...
      const fieldLabels = {
        testingDate: `提测时间`,
        releaseDate: `上线时间`,
        includeTestingDate: `开发工期是否包含提测日`,
        includeReleaseDate: `测试工期是否包含上线日`,
        testingStart: `提测开始`,
        testingEnd: `提测结束`,
        releaseStart: `上线开始`,
        releaseEnd: `上线结束`,
      ...
      for (const item of batchHistory) {
        ...
        for (const field of Object.keys(item.after ?? {})) {
          const formatValue = (value) => field.startsWith(`include`)
            ? pmDeliveryBoundaryLabel(value)
            : pmDeliveryFormatDate(value);
          historyItem.createEl(`strong`, {
            text: `${fieldLabels[field] ?? field}：${formatValue(item.before?.[field])} → ${formatValue(item.after?.[field])}`,
          });
        }
      }
    }
    ...
    saveButton.addEventListener(`click`, async () => {
      ...
      const changedFields = [`testingDate`, `releaseDate`, `includeTestingDate`, `includeReleaseDate`]
      ...
      if (changedFields.length > 0) {
        ...
        const confirmed = window.confirm(
          `本次时间或边界日口径调整将使迭代应完成度由 ${currentSummary.health.expected}% 变为 ${nextSummary.health.expected}%，`
...
function pmRenderDeliveryBatches(container, view, plan, statuses, manualDates) {
  ...
  for (const batch of plan.batches) {
    ...
    for (const [metricId, label, value, tone, iconName, calculation] of [
      ...
      [`testing-date`, `提测时间`, `${pmDeliveryFormatDate(batch.testingDate)} · 开发${pmDeliveryBoundaryLabel(batch.includeTestingDate)}`, ``, `calendar-clock`, ``],
      [`release-date`, `上线时间`, `${pmDeliveryFormatDate(batch.releaseDate)} · 测试${pmDeliveryBoundaryLabel(batch.includeReleaseDate)}`, ``, `calendar-check`, ``],
    ]) {
...
function pmIterationSummaryDeliverySignature(plan) {
  ...
  return JSON.stringify({
    batches: normalized.batches.map((batch) => ({
      ...
      includeTestingDate: batch.includeTestingDate,
      includeReleaseDate: batch.includeReleaseDate,
      owner: batch.owner,
...
function pmRenderIterationSummaryBatchMeta(container, batch) {
  ...
  for (const [label, value] of [
    [`负责人`, batch.owner || `未设置`],
    [`提测日期`, `${pmDeliveryFormatDate(batch.testingDate)} · 开发${pmDeliveryBoundaryLabel(batch.includeTestingDate)}`],
    [`上线日期`, `${pmDeliveryFormatDate(batch.releaseDate)} · 测试${pmDeliveryBoundaryLabel(batch.includeReleaseDate)}`],
  ]) {
...
function pmRenderIterationSummaryBatchOverview(
  ...
  for (const batch of deliveryPlan.batches) {
    ...
    metrics.createSpan({
      text: `提测 ${pmDeliveryFormatDate(batch.testingDate)}（开发${pmDeliveryBoundaryLabel(batch.includeTestingDate)}）`
        + ` · 上线 ${pmDeliveryFormatDate(batch.releaseDate)}（测试${pmDeliveryBoundaryLabel(batch.includeReleaseDate)}）`,
    });
...
function pmBuildIterationSummaryNoticeText({
  ...
  if (batch) {
    ...
    traceLines.push(`2. 提测日期：${pmDeliveryFormatDate(batch.testingDate)}（开发${pmDeliveryBoundaryLabel(batch.includeTestingDate)}）；`
      + `上线日期：${pmDeliveryFormatDate(batch.releaseDate)}（测试${pmDeliveryBoundaryLabel(batch.includeReleaseDate)}）；`);
  } else {
...
const PM_ITERATION_TABLE_COLUMN_WEIGHTS = new Map([
  [`事项 ID`, 10.5],
  [`事项`, 29],
  [`阶段`, 7],
  [`状态`, 6.5],
...
/**
 * 按信息量将可见列换算为百分比宽度，让表格始终铺满当前容器。
 */
function pmApplyIterationPercentageTableLayout(tableView) {
  ...
  for (const resizer of headerRow.querySelectorAll(`.pm-table-column-resizer`)) resizer.remove();

  const styleWindow = headerRow.ownerDocument.defaultView ?? window;
  const headers = [...headerRow.children].filter((cell) => (
    cell instanceof HTMLTableCellElement && styleWindow.getComputedStyle(cell).display !== `none`
  ));
  const weightedHeaders = headers.map((cell) => {
    const label = String(cell.textContent ?? ``).replace(/[↑↓]/gu, ``).trim();
    const weight = cell.hasClass(`pm-table-cell-select`)
      || cell === headerRow.lastElementChild
      ? 2.5
      : PM_ITERATION_TABLE_COLUMN_WEIGHTS.get(label) ?? 8;
    return { cell, label, weight };
  });
  const totalWeight = weightedHeaders.reduce((total, header) => total + header.weight, 0) || 1;

  for (const header of weightedHeaders) {
    const width = `${(header.weight / totalWeight * 100).toFixed(4)}%`;
    header.cell.style.width = width;
    header.cell.style.removeProperty(`min-width`);
    header.cell.style.removeProperty(`max-width`);
    if (header.cell.hasClass(`pm-table-cell-select`)) {
      table.style.setProperty(`--pm-freeze-select-width`, width);
    }
    if (header.label === `事项 ID`) table.style.setProperty(`--pm-freeze-id-width`, width);
  }

  table.removeClass(`pm-table--fixed-layout`);
  table.addClass(`pm-table--percentage-layout`);
  table.style.removeProperty(`--pm-table-fixed-width`);
}

const projectTableRender = Yp.prototype.render;

// 保留原有表格能力，补充百分比列布局和窗口化行高管理。
Yp.prototype.render = function render() {
  projectTableRender.call(this);
  pmApplyIterationPercentageTableLayout(this);
};
...
function pmSyncTableMultiSelectMode(view) {
  ...
  if (view.subview instanceof Yp) pmApplyIterationPercentageTableLayout(view.subview);
}
...
function pmMountIterationViewSwitcher(view) {
  const filterRow = view.contentEl.querySelector(`.pm-project-header-filter`);
  if (!(filterRow instanceof HTMLElement)) return;
  const mountedSwitcher = filterRow.querySelector(`.pm-iteration-view-switcher`);
  if (mountedSwitcher instanceof HTMLElement) {
    const mountedRight = filterRow.querySelector(`.pm-iteration-filter-layout-right`);
    if (mountedRight instanceof HTMLElement && mountedSwitcher.parentElement !== mountedRight) {
      mountedRight.appendChild(mountedSwitcher);
    }
    return;
  }

  const host = filterRow.querySelector(`.pm-iteration-filter-layout-right`) ?? filterRow;

  const switcher = new Fm(host, {
    options: [
...
function pmMountIterationFilterLayout(view) {
  ...
  const mountedRight = filterRow.querySelector(`.pm-iteration-filter-layout-right`);
  if (mountedRight instanceof HTMLElement) {
    const mountedLeft = filterRow.querySelector(`.pm-iteration-filter-layout-left`);
    if (mountedLeft instanceof HTMLElement) {
      // 原筛选组件局部刷新时会把新按钮追加回根节点，需要重新收拢到左侧布局。
      for (const element of Array.from(filterRow.children)) {
        if (element !== mountedLeft && element !== mountedRight) mountedLeft.appendChild(element);
      }
    }
    if (switcher.parentElement !== mountedRight) mountedRight.appendChild(switcher);
    return;
  }
...
const projectViewScheduleFilterRender = Um.prototype.scheduleFilterRender;

// 各视图的筛选更新路径不同，统一在内容刷新前恢复右侧类型筛选和视图入口。
Um.prototype.scheduleFilterRender = function scheduleFilterRender() {
  const result = projectViewScheduleFilterRender.call(this);
  pmMountTableMultiSelectToggle(this);
  pmMountIterationViewSwitcher(this);
  pmMountIterationFilterLayout(this);
  return result;
...
const projectViewHandleSavedViewSave = Um.prototype.handleSavedViewSave;
Um.prototype.handleSavedViewSave = async function handleSavedViewSave(name) {
  await projectViewHandleSavedViewSave.call(this, name);
  pmMountTableMultiSelectToggle(this);
  pmMountIterationViewSwitcher(this);
  pmMountIterationFilterLayout(this);
};

const projectViewHandleSavedViewDelete = Um.prototype.handleSavedViewDelete;
Um.prototype.handleSavedViewDelete = async function handleSavedViewDelete(savedViewId) {
  await projectViewHandleSavedViewDelete.call(this, savedViewId);
  pmMountTableMultiSelectToggle(this);
  pmMountIterationViewSwitcher(this);
  pmMountIterationFilterLayout(this);
};
...
Um.prototype.renderCurrentView = function renderCurrentView() {
  ...
  // 子视图重绘后再次确认筛选栏右侧入口仍在，避免切换看板时临时节点丢失。
  pmMountIterationViewSwitcher(this);
  pmMountIterationFilterLayout(this);
  pmSyncTableMultiSelectMode(this);
...
async function pmSetProjectArchived(context, project, archived) {
  ...
  window.dispatchEvent(new CustomEvent('pm-followed-projects-changed', {
    detail: { projectId: project.id, archived },
  }));
}
...
function pmRefreshCurrentProjectPage(view, button) {
  ...
  window.requestAnimationFrame(() => {
    ...
    if (view.subview instanceof Yp) {
      pmApplyIterationPercentageTableLayout(view.subview);
...
nh.prototype.getFollowedProjectSummaries = async function getFollowedProjectSummaries() {
  ...
  return projects
    .filter((project) => pmProjectFollowed(this, project) && !pmProjectArchived(context, project))
